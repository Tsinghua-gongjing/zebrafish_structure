...
#
#don't output scalingFactor of each window when using window-wise normalized RT file;
#calculate formula and winsorization doesn't change;
use strict;
use warnings;
use File::Basename;
use Getopt::Std;
...
sub winsorization{
    ...
    foreach my $trans ( keys %{$ref_transcript_signal} )  {
        ...
        if ( $winsorUpper == $winsorLower ) {
            ...
            #delete $ref_transcript_signal->{$trans};
            next;
        }
...
sub outputEnrichment  {
    ...
    foreach my $trans ( keys %{$ref_transcript_enrichment} )  {
        ...
        print OUT $trans, "\t", $ref_transcript_len->{$trans}, "\t", $ref_fg_rpkm->{$trans}, ",", $ref_bg_rpkm->{$trans}, "\t", ".";
        for ( my $idx = 0; $idx < $ref_transcript_len->{$trans}; $idx++ ) {
